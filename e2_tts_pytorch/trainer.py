...
class HFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample_rate = row['audio_path']['sampling_rate']
        duration = audio.shape[-1] / sample_rate

        if duration > 23 or duration < 0.3:
            logger.warning(f"Skipping due to duration out of bound: {duration}")
            return self.__getitem__((index + 1) % len(self.data))
        
        audio_tensor = torch.from_numpy(audio).float()
        if audio_tensor.ndim == 1:
            #logger.warning(f"Audio is not Mono Convert it {row['audio_path']['path']}")
            audio_tensor = audio_tensor.unsqueeze(0)  # Add channel dimension
        else:
            logger.warning(f"audio is mono")
...
class E2Trainer:
    def __init__(
        ...
        logger.add(log_file)
        self.name = name_checkpoint_path
